...
Пример отображения:
Инв. №100001 | на складе | [Показать QR] [Открыть карточку]

---

### Карточка экземпляра оборудования

У каждого экземпляра оборудования есть собственная карточка.

Карточка используется для:
- хранения физических характеристик
- визуальной идентификации оборудования
- быстрого доступа со склада и из мероприятий

Карточка экземпляра содержит:
- инвентарный номер (read-only)
- текущий статус
- фотографию оборудования
- вес
- дополнительные параметры (будут уточняться по мере необходимости)

Карточка экземпляра:
- не используется при планировании мероприятий
- не влияет на расчёт доступности
- используется кладовщиком и техническим персоналом
...
Статус напрямую влияет на доступное количество.
...
- комплектность — связь между экземплярами, а не иерархия типов
...
2. Экземпляры не становятся объектами планирования
3. Карточка экземпляра — для информации и истории
